...
try {
  ...
  // ---------- 4a. 落点后按区域自动分类 ----------
  // 落点后：答案位置回显「自动判定区域」，并且分类下拉被自动设成同一个区域。
  // （判定用的推断规则与 scripts/classify-regions.mjs 是同一份实现，见 shared/regionInference）。
  {
    const auto = await page.evaluate(() => {
      const select = document.querySelector('[data-testid="field-category"]')
      const slot = document.querySelector('[data-testid="answer-slot"]')
      const values = [...(slot?.querySelectorAll('strong') ?? [])].map((node) => node.textContent.trim())
      return {
        regionText: values[2] || '',
    ...
    check(Boolean(regionLabel) && (auto.categoryText || '').includes(regionLabel),
      `自动分类与自动判定区域一致 => 区域「${regionLabel}」vs 分类「${auto.categoryText}」`)
  }
